...
- [Arch Linux](#arch-linux)
  ...
  - [pacman](#pacman)
    ...
    - [更多的软件仓库](#更多的软件仓库)
      ...
      - [Arch 用户软件仓库 (AUR)](#arch-用户软件仓库-aur)
        ...
        - [从github备用仓库安装AUR软件包](#从github备用仓库安装aur软件包)
  ...
  - [技巧](#技巧)
    ...
    - [设置 disk 权限](#设置-disk-权限)
    - [指定 chromium 系软件使用 kwallet](#指定-chromium-系软件使用-kwallet)
    - [挂载 smb 上的目录](#挂载-smb-上的目录)
    - [htop/btop](#htopbtop)
  - [Hacker](#hacker)
    - [清除登录记录](#清除登录记录)
    - [USB/PCI 设备远程热插拔](#usbpci-设备远程热插拔)
      - [USB 虚拟拔插](#usb-虚拟拔插)
      - [PCI 虚拟拔插](#pci-虚拟拔插)
  - [Wiki](#wiki)
...
> [!NOTE]
> 文中会出现一些特殊格式, 例如 `<xxx>` 代表根据实际情况填写的必填项, `[xxx]` 代表可选项, 请根据上下文自行判断  
...
开机引导方式 (BIOS/UEFI), 分区表类型(MBR/GPT), 分区信息
...
你可以按照下方步骤在 archiso 内完成创建和格式化分区,
或在功能正确的图像化界面下分区, 然后回到这里从挂载分区开始, 例如 Linux 下的 `GParted`, `Partition Manager`
...
- mesa
  ...
  ```bash
  # 被 x11 和 wayland 依赖, 图形界面用户无须自行安装
...
- Intel
  ...
  i915 驱动包含在内核模块中, 开箱即用

  - xe

    [如何使用新的 xe 实验性驱动](#intel-xe)
...
/dev/sda2 /mnt/excess ntfs-3g uid=1000,gid=1000,dmask=027,fmask=137    0 2
```
...
确保能完整休眠至少需要的 swap 大小

```bash
numfmt --to=iec-i --format="%.2f" $(cat /sys/power/image_size)
```
...
[前往 Performance Optimization 章节](./performance-optimization.md)
...
`/etc/tmpfiles.d/xxx.conf`

```conf
类型  路径           模式  用户  用户组  老化时间  内容
w     /path/to/file  -     -     -       -         defer+madvise
```
...
github aur仓库为只读镜像仓库, 只在 aur 因种种原因(如被DDOS)无法访问时, 考虑此备用方式安装

```bash
PACKAGE_NAME=<package_name>
git clone https://github.com/archlinux/aur.git --depth=1 --branch ${PACKAGE_NAME} ${PACKAGE_NAME}
cd ${PACKAGE_NAME}
makepkg -si
```
...
| `nethogs`                 | 网络流量监听                          |
| `ethtool`                 |                                       |
| `wireshark/wireshark-cli` | 网络分析工具                          |
| [`samba`](#samba)         | 文件共享                              |
| `rustscan`                | 端口扫描                              |
| `httping`                 | http ping                             |
| `somo`                    | 查看进程端口                          |
...
- 通用
  ...
  安装 switcheroo-control, 运行 `switcheroo-control list` 获取对应显卡的环境变量，然后配置即可
...
```bash
udevadm info --query=all --name=/dev/nvme1n1 | grep ID_SERIAL=
```

在 `/etc/udev/rules.d/99-disk.rules` 添加如下内容

```bash
ENV{ID_SERIAL}=="<你的设备序列号>", OWNER="<你的用户名>", MODE="0660"
```
...
### 指定 chromium 系软件使用 kwallet

在命令行或 .desktop 中添加启动参数

```bash
--password-store=kwallet5
```

vscode 则在命令面板中搜索 `Preferences: Configure Runtime Arguments` 并添加
...
```bash
sudo mount --mkdir -t cifs \
  '//192.168.110.14/<folder>' \
  /mnt/<folder> \
...
按 `F` (大写) 即可进入 follow 状态 (pid 跟踪)

htop 可以通过 F4 搜索来筛选并锁定进程
...
```bash
# Remove
echo 1 | sudo tee /sys/bus/usb/devices/<USB_PATH>/remove
# Rescan
...
- 从输入事件反查
  ...
  ```bash
  udevadm info -q path -n /dev/input/eventX
  ```

- 从 lsusb 反查

  ```bash
  udevadm info --query=path --name=/dev/bus/usb/<bus>/<dev>
  ```

- 查看 udev 属性

  ```bash
  udevadm info --attribute-walk --name=/dev/bus/usb/<bus>/<dev>
  ```
